# swaync ported from anotherhadi/nixy (home/system/swaync/default.nix).
# stylix colours / config.theme are replaced by the local theme.
{ pkgs, scripts, ... }:
let
  t = import ./theme.nix;
  c = t.colors;
  font = t.fonts.sansSerif;
  monofont = t.fonts.monospace;
  rounding = t.rounding;
  borderSize = t.borderSize;
  gapsIn = t.gapsIn;
  opacityToHex = opacity: let
    val = builtins.floor (opacity * 255);
    high = builtins.floor (val / 16);
    low = val - high * 16;
    digits = "0123456789abcdef";
  in "${builtins.substring high 1 digits}${builtins.substring low 1 digits}";
  alphaHex = opacityToHex t.inactiveOpacity;
in
{
  services.swaync = {
    enable = true;
    ...
    settings = {
      ...
      "control-center-margin-top" = t.gapsOut * 2;
      "control-center-margin-bottom" = t.gapsOut * 2;
        ...
        "buttons-grid" = {
          actions = [
            ...
            }
            {
              ...
              "update-command" = "test -f $XDG_RUNTIME_DIR/hypr-focus-mode && echo true || echo false";
            }
            {
              label = "󰍭";
              type = "toggle";
    ...
    style = ''
      ...
      * {
        ...
        --border-radius-s: ${toString gapsIn}px;
      ...
      .notification-background {
        ...
        border: ${toString borderSize}px solid @selected;
        box-shadow: none;
        outline: none;
      ...
      .notification-background .close-button {
        margin: 6px;
        padding: 2px;
        border-radius: ${toString (gapsIn - 2)}px;
        background: transparent;
        color: @text;
      }

      .notification-background .close-button:hover { background: @hover; }
      .notification.low, .notification.normal { border: none; }

      .notification.critical {
        border: ${toString borderSize}px solid @urgent;
        border-radius: var(--border-radius);
      }
      ...
      .notification-content .time { padding: 2px; font-size: 0.95rem; font-weight: 800; }
      ...
      .notification-content .body { color: @text-alt; font-size: 0.8rem; }
      ...
      .notification-action:hover, .notification-action:active { background: @selected; }

      .control-center {
        background: @center-bg;
        border-radius: var(--border-radius) 0 0 var(--border-radius);
        border-left: ${toString borderSize}px solid @selected;
        margin: 0;
        padding: 5px 5px 0 5px;
      }

      .control-center .notification-background { background: @center-notification-bg; }
      ...
      .notification-group:not(.collapsed) { margin: ${toString gapsIn}px; }

      .notification-group-headers, .notification-group-buttons {
        color: @text;
        font-weight: bold;
      ...
      .notification-group-headers > label { margin: 0 3px; font-size: 1rem; }
      ...
      .widget { margin: 5px; color: @text; }
      .widget-mpris-album-art { border-radius: var(--border-radius); margin: 6px 4px; }
      .widget-mpris-title { font-weight: 700; font-size: 1.2rem; margin: 0 3px; }
      .widget-mpris-subtitle { color: @text-alt; font-weight: 700; font-size: 0.9rem; margin: 0 3px; }
      .mpris-background { border-radius: 14px; margin: 3px; }
      .widget-mpris { border-radius: var(--border-radius); }
      ...
      .widget-mpris-player button { padding: 5px; margin: 0 2.5px; }
      .widget-mpris-player button:hover { border-radius: var(--border-radius-s); background: @hover; }
      ...
      .widget-volume > box, .widget-backlight > box { color: @text; }
      .widget-volume label, .widget-backlight label { font-size: 1rem; font-weight: 700; min-width: 1.2rem; }
      ...
      .widget-dnd > label { font-size: 1rem; font-weight: 700; }
      .widget-dnd switch {
        background: @hover;
        border-radius: var(--border-radius-s);
        box-shadow: none;
      ...
      .widget-dnd switch slider { background: @text; border-radius: var(--border-radius-s); }
      .widget-dnd switch:hover, .widget-dnd switch:checked { background-color: @selected; }
  ...
  # GTK 4.22 Vulkan renderer stalls on first control-center open; use GL (ngl).
