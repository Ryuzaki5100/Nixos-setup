# Helper scripts ported from anotherhadi/nixy (home/system/waybar/scripts.nix).
# Only the stylix font reference is replaced by the local theme; the rest is
# kept upstream. Returned as an attrset and exposed via _module.args.scripts.
{ pkgs, ... }:
let
  t = import ./theme.nix;
...
in
{
  ...
  icon-picker = pkgs.writeShellScriptBin "icon-picker" ''
    selected=$(${tofiMenu} --font "${t.fonts.monospace}" --prompt-text 'icon: ' < ${nerdFontIconList})
